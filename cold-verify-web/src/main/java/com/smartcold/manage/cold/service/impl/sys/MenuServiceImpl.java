...
import com.smartcold.manage.cold.dao.sys.MenuDao;
import com.smartcold.manage.cold.dao.sys.RoleMenuDao;
...
public class MenuServiceImpl implements MenuService {
	...
	@Cacheable
	@Override
	public TreeModel<MenuDO> getSysMenuTree(Long id) {
		List<TreeModel<MenuDO>> trees = new ArrayList<TreeModel<MenuDO>>();
		List<MenuDO> menuDOs = menuMapper.listMenuByUserId(id);
		for (MenuDO sysMenuDO : menuDOs) {
			TreeModel<MenuDO> tree = new TreeModel<MenuDO>();
		...
		TreeModel<MenuDO> t = BuildTree.build(trees);
		return t;
	}
	...
	@Override
	public TreeModel<MenuDO> getTree() {
		List<TreeModel<MenuDO>> trees = new ArrayList<TreeModel<MenuDO>>();
	...
	@Override
	public TreeModel<MenuDO> getTree(Long id) {
		// 根据roleId查询权限
	...
	@Override
	public List<TreeModel<MenuDO>> listMenuTree(Long id) {
		List<TreeModel<MenuDO>> trees = new ArrayList<TreeModel<MenuDO>>();
		List<MenuDO> menuDOs = menuMapper.listMenuByUserId(id);
		for (MenuDO sysMenuDO : menuDOs) {
			TreeModel<MenuDO> tree = new TreeModel<MenuDO>();
		...
		// 默认顶级菜单为０，根据数据库实际情况调整
		List<TreeModel<MenuDO>> list = BuildTree.buildList(trees, "0");
		return list;
	}
